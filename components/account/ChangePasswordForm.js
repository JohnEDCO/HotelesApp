...
export default function ChangePasswordForm({ setShowModal, toastRef }) {
    ...
    const onSubmit = async() => {
        ...
        toastRef.current.show("Se ha actualizado correctamente", 3000)
        setShowModal(false)
    }
    ...
    const validateForm = () => {
        ...
        if(size(newPassword) < 6) {
            setErrorNewPassword("Ingresa una nueva contraseña con almenos 6 carácteres.")
            isValid = false
        }

        if(size(confirmPassword) < 6) {
            setErrorConfirmPassword("Ingresa una nueva confirmación de tu contraseña con almenos 6 carácteres.")
            isValid = false
        }
